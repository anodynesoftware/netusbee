...
main(int argc, char **argv)
{
	...
	/* Get _USB cookie */
	api = (void *)Supexec(get_usb_cookie);
	if (!api)
	{
		c_conws ("NetUSBee failed to get _USB cookie\r\n");
		return -1;
	}

	/* for (fairly) precise mdelay/udelay relative to CPU power */
	init_delay((int)Supexec(get_cpu_cookie));
	Supexec(calibrate_delay);

#ifndef SLOW_MACHINE
	delay_150ns = loopcount_1_msec * 15 / 100000L;
	delay_300ns = loopcount_1_msec * 3 / 10000L;
#endif
	delay_1us = loopcount_1_msec / 1000;
